			...
			if (!source.equals("versatile-heritages.json")) {
				...
				features.add(
						new FeatureEmbeddable(
								"Flavor",
								"<p>"
										.concat(String.join("</p><p>", ancestryImport.getFlavor()))
										.concat("</p>")));
				features.add(
						new FeatureEmbeddable(
								"Info",
						...
						.forEach(
								i -> {
									...
									if (info.contains("entries=[") && info.contains("name=")) {
										...
										String entries =
												info.substring(info.indexOf("entries=[") + 9);
										if (entries.contains("{type=list")) {
											entries =
													entries.substring(
															entries.indexOf("items=[") + 7);
											entries =
													"<ul><li>"
															.concat(
																	entries.substring(
			...
			for (String boost : boosts) {
				AbilityBoostEmbeddable abilityBoost =
						new AbilityBoostEmbeddable(
								boost.contains("free")
										? null
										: AbilityScoreEnum.valueOf(
												boost.substring(0, 3).toUpperCase()),
								boost.contains("free"),
								flaw);
